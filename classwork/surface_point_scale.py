...
def SurfacePoints(STRSRF, INTU, INTV, ATTRPT):
    ...
    for i in range(INTU+1):
        for j in range(INTV+1):
            ...
            #measure distance from matrix point to attractor point 
            distance = rs.Distance(point, ATTRPT)
            #print distance
            
            #find surface normal(vector) at parameter
            vecNorm = rs.SurfaceNormal(STRSRF, (u, v))
            #print vecNorm
            #scale normal based on distance value
            vecNorm = rs.VectorScale(vecNorm, distance/5)
